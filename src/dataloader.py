...
class SNV3Dataset(Dataset):
    def __init__(
        ...
        for game in tqdm(self.list_games):
            ...
            try:
                obj_reference = self.s3.Object(
                    "soccernet-230", "caption-2023/" + game + "/Labels-caption.json"
                )
        ...
        self.text_processor = SoccerNetTextProcessor(
            self.getCorpus(split=[split]), split=split, load_path=vocab_path
        )
    ...
    def __getitem__(self, index):
        ...
        label_reference = self.s3.Object(
            "soccernet-230", "caption-2023/" + game + "/Labels-caption.json"
        )
        ...
        emb_ref1 = self.s3.Object(
            "soccernet-230", "caption-2023/" + game + "/1_baidu_soccer_embeddings.npy"
        )
        emb_ref2 = self.s3.Object(
            "soccernet-230", "caption-2023/" + game + "/2_baidu_soccer_embeddings.npy"
        )
            ...
            for v_id in [1, 2]:
                if self.local_video_path:
                    video = imageio.get_reader(
                        f"{local_video_path}/{game}/{v_id}_224p.mkv", "ffmpeg"
                    )
                else:
        ...
        for annotation in captions:
            ...
            if self.include_vid:
                out["clip"] = []
                if (
                    time_in_sec + self.window_size
                    < video_list[0].get_meta_data()["duration"]
                ):
                    video = video_list[0]
                else:
                    video = video_list[1]
                    ...
                    frame = int(
                        self.framerate
                        * (time_in_sec - video_list[0].get_meta_data()["duration"])
                    )
                    start = min(frame, embed.shape[0] - self.clip_len)
                ...
                frame_indexes = [
                    i
                    for i in range(int(video.count_frames()))
                    if i % sample_interval == 0
                ]
                clip = [
                    torch.tensor(video.get_data(frame_indexes[i]))
    ...
    def detokenize(self, tokens, remove_EOS=True):
        ...
        return (
            string.rstrip(
                f" {self.text_processor.vocab.lookup_token(config.EOS_TOKEN)}"
            )
            if remove_EOS
            else strin
        )
...
def collate_fn(batch):
    ...
    data["caption"], data["mask"], data["length"] = collate_fn_padd(data["caption"])
...
def collate_fn_padd(batch):
    ...
    ## padd
    tokens = [
        ([config.SOS_TOKEN] + t + [config.EOS_TOKEN])
        if t
        else [config.PAD_TOKEN, config.PAD_TOKEN]
        for t in batch
    ]
